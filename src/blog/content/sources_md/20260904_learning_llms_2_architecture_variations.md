...
Every experiment here changed a part I had already built. Next I added one I
hadn't: a Mixture of Experts layer, which swaps the single MLP in each block for
several, plus a small router that picks which one each token uses.
